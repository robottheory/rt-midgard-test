...
func runActionsQuery(ctx context.Context, q preparedSqlStatement) ([]action, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("actions process: %w", err)
		}
...
func GetActions(ctx context.Context, moment time.Time, params ActionsParams) (
	...
	// EXECUTE QUERIES
	countPS, resultsPS, optimizedResultsPS, err := actionsPreparedStatements(
		moment,
		params.TXId,
		addresses,
		params.Asset,
	...
	// Get results subset
	actions, err := runActionsQuery(ctx, optimizedResultsPS)
	if err != nil {
		return oapigen.ActionsResponse{}, err
	}

	if len(actions) < int(limit) && len(actions) < int(txCount) {
		// Optimized query didn't return enough results, run the full query:
...
const actionsOptimizedInterval = 604800e9 // one week (in nanoseconds)
...
// A third query is also returned, which is exactly like the results query, but with timestamp
// limited to a one week interval. This query is usually _much_ faster and in most common cases
...
func actionsPreparedStatements(moment time.Time,
	...
	var countPS, resultsPS, optimizedResultsPS preparedSqlStatement
	...
	// (txid, address, asset)
	whereQuery := `
	WHERE union_results.block_timestamp <= #MOMENT# ##AND_OPTIMIZED##`
	...
	countQuery = strings.ReplaceAll(countQuery, "##AND_OPTIMIZED##", "")
	...
	optimizedResultsQuery := strings.ReplaceAll(resultsQuery, "##AND_OPTIMIZED##",
		fmt.Sprintf("AND $%d < union_results.block_timestamp", len(resultsQueryValues)+1))

	resultsQuery = strings.ReplaceAll(resultsQuery, "##AND_OPTIMIZED##", "")
	resultsPS = preparedSqlStatement{resultsQuery, resultsQueryValues}

	optimizedResultsQueryValues := append(resultsQueryValues, moment.UnixNano()-actionsOptimizedInterval)
	optimizedResultsPS = preparedSqlStatement{optimizedResultsQuery, optimizedResultsQueryValues}

	return countPS, resultsPS, optimizedResultsPS, nil
}
